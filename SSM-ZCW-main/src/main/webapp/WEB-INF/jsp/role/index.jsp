            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    ...
                    <button type="button" class="btn btn-primary" style="float:right;"
                            onclick="window.location.href='${APP_PATH}/role/toAdd.do'">
                        <i class="glyphicon glyphicon-plus"></i> 新增
                    </button>
                    <br>
                    <hr style="clear:both;">
                    <div class="table-responsive">
                        <table class="table  table-bordered" id="role_teable">
                            <thead>
                            <tr>
                            ...
                            <tfoot>
                            <tr>
                                <td colspan="4" align="center" id="page_nav"></td>
                            </tr>
                            </tfoot>
                        </table>
                    </div>
...
<script type="text/javascript">
    ...
    function roleTable(result) {
        ...
        $.each(data, function (index, role) {
            content += '<tr>';
            content += '	<td><input type="checkbox" class="checkDel" index=' + role.id + '></td>';
            content += '	<td>' + role.id + '</td>';
            content += '	<td>' + role.name + '</td>';
            content += '	<td>';
            content += '		<button type="button" class="btn btn-success btn-xs" onclick="toAssignPermission(' + role.id + ')"><i class=" glyphicon glyphicon-check"></i></button>';
            content += '		<button type="button" class="btn btn-primary btn-xs editUserBtn" onclick="toUpdate(' + role.id + ')"><i class=" glyphicon glyphicon-pencil"></i></button>';
            content += '		<button type="button" class="btn btn-danger btn-xs delUserBtn" onclick="del(' + role.id + ',\'' + role.name + '\')" ><i class=" glyphicon glyphicon-remove"></i></button>';
            content += '	</td>';
            content += '</tr>';
        });
        $("#role_teable tbody").html(content);
    }
    ...
    function del(id, name) {
        layer.confirm("您确定要删除用户[" + name + "]吗？", {icon: 3, title: '提示'}, function (cindex) {
            layer.close(cindex);
            $.ajax({
                url: "${APP_PATH}/role/del.do",
                data: {"id": id},
                type: "POST",
                success: function (result) {
                    if (result.success) {
    ...
    $("#delPoleAll").click(function () {
        ...
        layer.confirm("您确定要删除这些角色吗？", {icon: 3, title: '提示'}, function (cindex) {
            layer.close(cindex);
            let jsonObj = {};
            $.each(roles, function (inedx, role) {
                jsonObj["ids[" + inedx + "]"] = $(role).attr("index");
            });
            console.log(jsonObj);
            $.ajax({
                url: "${APP_PATH}/role/delBatchRole.do",
                data: jsonObj,
                type: "POST",
                success: function (result) {
                    if (result.success) {
